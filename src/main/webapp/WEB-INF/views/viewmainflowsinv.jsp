...
<head>
    ...
    <style type="text/css">
        ...
        .d24 {
            ...
            position: relative; left: 0%;margin-right: 10%;margin-left:0;}
        .d24:after {
            content: "";
...
<div class="generic-container" style="height: 95%">
    ...
    <form:form modelAttribute="searchSummaryF" method="POST" id="searchFacility" class="form-inline">
        <div class="row" style="margin: 10px 10px 10px 0; padding-left: 9%">
            ...
            <form:select path="facility" id="facilities" items="${facilities}" multiple="false" class="form-control input-sm" />
            <button type="submit" name="facilitySubmit" id="bth-search" class="btn btn-primary btn-sm">Фильтр</button>
        </div>
    </form:form>

    <div class="row">
        <div class="col-md-6" style="margin-bottom: 10px; height: 50%; overflow-y: scroll; padding-left: 10%; padding-right: 10%">
            <div class="panel panel-default">
                ...
                <form:form modelAttribute="searchSummary" method="POST" class="form-inline" id="search-form-months">
                    <div class="row" style="margin: 10px;">
                        ...
                        <form:select path="pay" id="paysMonth" items="${pays}" multiple="false" class="form-control input-sm" />
                        <label class="sr-only" for="underFacilitiesMonth">Подобъект:</label>
                        <form:select path="underFacility" id="underFacilitiesMonth" items="${uFacilities}" multiple="false" class="form-control input-sm" />
                        <button type="submit" name="allSubmit" id="bth-search" class="btn btn-primary btn-sm">Фильтр</button>
                    </div>
                </form:form>
                        ...
                        <tr>
                            ...
                            <c:choose>
                                <c:when test="${flows.summa >= 0}">
                                    <fmt:formatNumber var="percent" value="${(flows.summa / finalMaxSum)*100}" maxFractionDigits="0"/>
                                ...
                                <c:when test="${flows.summa < 0}">
                                    <fmt:formatNumber var="percent" value="${(flows.summa / finalMinSum)*100}" maxFractionDigits="0"/>
                                    <fmt:formatNumber var="middle" value="${(percent + 100)/2}" maxFractionDigits="0"/>
        ...
        <div class="col-md-6" style="height: 50%; overflow-y: scroll; padding-right: 10%; padding-left: 10%; padding-top: 5%">
            <div class="panel panel-default">
                <!-- Default panel contents -->
                    ...
                    <c:forEach items="${invCash}" var="cash">
                        <tr>
                            ...
                            <td style="padding: 0"><a id="goCash" name="${cash.facility.facility}"
                                   href="<c:url value='/sumdetails' />" class="btn btn-link custom-width" style="padding: 0">Подробно</a></td>
                        </tr>
                    </c:forEach>
                    <tr>
                        <td style="text-align: center; font-weight: bold; padding: 0">Итого:</td>
    ...
    <div class="row">
            ...
            <div class="panel panel-default">
                ...
                <form:form modelAttribute="searchSummary" method="POST" class="form-inline" id="search-form">
                    <div class="row" style="margin: 10px;">
                        ...
                        <form:select path="iYear" id="years" items="${years}" multiple="false" class="form-control input-sm" />
                        <label class="sr-only" for="months">Месяц:</label>
                        <form:select path="iMonth" id="months" items="${months}" multiple="false" class="form-control input-sm" />
                        <label class="sr-only" for="pays">Платёж:</label>
                        <form:select path="pay" id="pays" items="${pays}" multiple="false" class="form-control input-sm" />
                        <label class="sr-only" for="underFacilities">Подобъект:</label>
                        <form:select path="underFacility" id="underFacilities" items="${uFacilities}" multiple="false" class="form-control input-sm" />
                ...
                <table class="table table-hover" id="allInvFlows">
                    ...
                    <c:forEach items="${allMainFlows}" var="flows">
                        <tr>
                            ...
                            <c:choose>
                                <c:when test="${flows.summa >= 0}">
                                    <fmt:formatNumber var="percent" value="${(flows.summa / maxSum)*100}" maxFractionDigits="0"/>
                                    <fmt:formatNumber var="middle" value="${(percent + 100)/2}" maxFractionDigits="0"/>
                                    <td style="background: linear-gradient(to right, white 50%, rgb(100, 195, 132) 50%, rgb(229, 245, 234) ${middle}%,
                                            white ${percent}%); text-align: right; padding: 0">
                                        <fmt:setLocale value = "ru-RU" scope="session"/>
                                ...
                                <c:when test="${flows.summa < 0}">
                                    <fmt:formatNumber var="percent" value="${(flows.summa / minSum)*100}" maxFractionDigits="0"/>
                                    <fmt:formatNumber var="middle" value="${(percent + 100)/2}" maxFractionDigits="0"/>
                                    <td style="background: linear-gradient(to left, white 50%, rgb(255, 0, 0) 50%, rgb(255, 210, 210) ${middle}%,
                                            white ${percent}%); text-align: right; padding: 0">
                                        <fmt:setLocale value = "ru-RU" scope="session"/>
        ...
        <div class="col-md-6" style="height: 50%; overflow-y: scroll; padding-right: 10%; padding-left: 10%; padding-top: 5%">
            <div class="panel panel-default">
                <!-- Default panel contents -->
                <div class="panel-heading"><span class="lead" style="font-weight: bold">Если объект продать, то Вы получите:</span></div>
                <table class="table table-hover" id="totals">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="padding: 0; text-align: right"><fmt:formatNumber value="1" type="currency" minFractionDigits="2"/></td>
                            <td style="padding: 0"><a id="go" name="тест"
                                   href="<c:url value='#' />" class="btn btn-link custom-width" style="padding: 0">Подробно</a></td>
                        </tr>
