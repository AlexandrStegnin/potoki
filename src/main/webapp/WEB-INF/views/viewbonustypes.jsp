...
<div class="generic-container">
    ...
    <div class="panel panel-default">
        ...
        <table class="table table-hover">
            ...
            <tbody>
            <c:forEach items="${bonusTypes}" var="bonus">
                <tr>
                    <td>${bonus.id}</td>
                    <td>${bonus.bonusType}</td>
                    <sec:authorize access="isFullyAuthenticated()">
                        <sec:authorize access="hasRole('ADMIN') or hasRole('DBA')">
                            <td><a href="<c:url value='/edit-bonustypes-${bonus.id}' />" class="btn btn-success custom-width">Изменить</a></td>
                        </sec:authorize>
                        <sec:authorize access="hasRole('ADMIN')">
                            <td><a href="<c:url value='/delete-bonustypes-${bonus.id}' />" class="btn btn-danger custom-width">Удалить</a></td>
                        </sec:authorize>
                    </sec:authorize>
                </tr>
            </c:forEach>
    ...
    <sec:authorize access="isFullyAuthenticated()">
        <sec:authorize access="hasRole('ADMIN')">
            <div class="well">
                <a href="<c:url value='/newbonustypes' />">Добавить бонус</a>
            </div>
        </sec:authorize>
    </sec:authorize>
</div>
