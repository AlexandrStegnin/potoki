...
<div class="generic-container">
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-heading"><span class="lead">Источник получения денег:</span></div>
        <table class="table table-hover" id="tblCashSrc">
            <thead>
            <tr>
                <th>№ п/п</th>
            ...
            <c:forEach items="${cashSources}" var="cashSource">
                <tr id="${cashSource.id}">
                    <td>${cashSource.id}</td>
                    <td>${cashSource.cashSource}</td>
                    <sec:authorize access="isFullyAuthenticated()">
                        <sec:authorize access="hasRole('ADMIN') or hasRole('DBA')">
                            <td style="text-align: center"><a href="<c:url value='/edit-cashsource-${cashSource.id}' />" class="btn btn-success custom-width">Изменить</a></td>
                        </sec:authorize>
                        <sec:authorize access="hasRole('ADMIN')">
                            <td style="text-align: center"><a href="<c:url value='/deletesource'/>" id="delete" name="${cashSource.id}"
    ...
    <sec:authorize access="isFullyAuthenticated()">
        <sec:authorize access="hasRole('ADMIN')">
            <div class="well">
                <a href="<c:url value='/newcashsource' />">Добавить источник</a>
            </div>
        </sec:authorize>
    </sec:authorize>
</div>
