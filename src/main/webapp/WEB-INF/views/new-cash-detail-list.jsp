...
<head>
    ...
    <title>Детали новых денег</title>
    <sec:csrfMetaTags/>
    <link rel="stylesheet"
        ...
        <sec:authorize access="isFullyAuthenticated()">
            <sec:authorize access="hasRole('ADMIN')">
                <div style="padding: 5px; margin-left: auto">
                    <a href="<c:url value='/#' />" id="create-new-details"
...
<div class="container-fluid">
    <table class="table table-striped w-auto table-hover table-sm" style="table-layout: fixed"
        ...
        <c:forEach items="${newCashDetails}" var="detail">
            <tr id="${detail.id}">
                <td>${detail.id}</td>
                <td>${detail.name}</td>
                <sec:authorize access="isFullyAuthenticated()">
                        ...
                        <td style="text-align: center">
                            <div class="dropdown pull-right" style="margin-right: 10px">
                                ...
                                <div class="dropdown-menu">
                                    <a class="dropdown-item" id="edit-detail" data-detail-id="${detail.id}"
                                       href="<c:url value='#' />">Изменить</a>
                                    <a class="dropdown-item" id="delete-detail" href="<c:url value='#' />"
                                       data-detail-id="${detail.id}" style="color: red">Удалить</a>
                                </div>
                            </div>
                        </td>
                    </sec:authorize>
...
<%@include file="new-cash-detail-form.jsp" %>
