            ...
            <c:forEach items="${typeClosingInvestsList}" var="types">
                <tr id="${types.id}">
                    ...
                    <sec:authorize access="isFullyAuthenticated()">
                        <sec:authorize access="hasRole('ADMIN') or hasRole('DBA')">
                            <td style="text-align: center"><a href="<c:url value='/edit-typeClosing-${types.id}' />" class="btn btn-success custom-width">Изменить</a></td>
                        </sec:authorize>
                        <sec:authorize access="hasRole('ADMIN')">
                            <td style="text-align: center"><a href="<c:url value='/deleteTypeClosing'/>" id="delete" name="${types.id}"
